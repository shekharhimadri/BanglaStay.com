    ...
    <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-stone-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-3">
            <button className="text-sm font-medium text-stone-600 hover:text-green-700 transition-colors">Sign in</button>
            <button className="text-sm font-semibold px-4 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700 transition-colors shadow-sm">
              Register
            </button>
          </div>
      ...
      {open && (
        <div className="md:hidden border-t border-stone-200 bg-white px-4 py-4 space-y-3">
          ...
          <button className="w-full text-sm font-semibold px-4 py-2 rounded-lg bg-green-600 text-white">Register</button>
        </div>
      )}
    </header>
  );
...
function SearchBar() {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-2 sm:p-3 flex flex-col md:flex-row md:items-center gap-2 md:gap-0 max-w-4xl mx-auto">
      <div className="flex items-center gap-3 flex-1 px-4 py-3 md:border-r md:border-stone-200">
        ...
        <div className="flex-1 text-left">
          ...
          <input
            ...
            placeholder="Where in Bangladesh?"
            className="w-full text-sm font-medium text-stone-800 placeholder:text-stone-400 outline-none bg-transparent"
          />
        </div>
      </div>
      <div className="flex items-center gap-3 flex-1 px-4 py-3 md:border-r md:border-stone-200">
      ...
      <button className="flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white font-semibold px-6 py-3 rounded-xl transition-colors shadow-sm">
  ...
  return (
    <section id="destinations" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
      <div className="flex items-end justify-between mb-8">
        ...
        <a href="#stays" className="hidden sm:flex items-center gap-1 text-sm font-semibold text-green-700 hover:text-green-800 transition-colors">
    ...
    <article className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-stone-100 flex flex-col">
      <div className="relative aspect-[16/10] overflow-hidden">
        ...
        <button
          ...
          aria-label="Save"
        >
          <Heart className={`w-4 h-4 ${liked ? 'fill-red-500 text-red-500' : 'text-stone-500'}`} />
        </button>
      </div>
...
/* ---------- Stays ---------- */
function Stays({ onBook }: { onBook: (h: Hotel) => void }) {
  const [filter, setFilter] = useState<string>('all');
  const filtered = useMemo(() => {
    if (filter === 'all') return hotels;
    return hotels.filter((h) => h.destinationId === filter);
  }, [filter]);
  return (
    <section id="stays" className="bg-stone-50 py-16 sm:py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-stone-500">Handpicked properties loved by travellers</p>
        </div>
        <div className="flex flex-wrap gap-2 mb-8">
          <FilterChip label="All stays" active={filter === 'all'} onClick={() => setFilter('all')} />
          {destinations.map((d) => (
            <FilterChip key={d.id} label={d.name} active={filter === d.id} onClick={() => setFilter(d.id)} />
          ))}
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((h) => (
            <HotelCard key={h.id} hotel={h} onBook={onBook} />
          ))}
        </div>
      </div>
    </section>
    ...
    <section id="deals" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 sm:pb-20">
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-green-700 via-green-700 to-emerald-800 p-8 sm:p-12">
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <button className="self-start md:self-center bg-white text-green-700 font-semibold px-6 py-3 rounded-xl hover:bg-green-50 transition-colors shadow-sm whitespace-nowrap">
            Browse deals
          </button>
        </div>
      </div>
...
/* ---------- Booking Modal ---------- */
function BookingModal({ hotel, onClose }: { hotel: Hotel | null; onClose: () => void }) {
        ...
        {confirmed ? (
          ...
          <div className="p-6">
            ...
            <button
              ...
              className="mt-5 w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-3 rounded-xl transition-colors shadow-sm"
            >
              Confirm booking · ৳{formatBDT(grand)}
            </button>
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-white text-stone-900 antialiased">
      ...
      <Hero />
      <Destinations />
      <Stays onBook={setBooking} />
      <Experiences />
      <Deals />
      <Footer />
      <BookingModal hotel={booking} onClose={() => setBooking(null)} />
    </div>
  );
}
